...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
	...
	private static long portTxThreshold=100;
	private static long portRxThreshold=100;
	...
	private class PortStatsCollector implements Runnable {
		...
		@Override
		public void run() {
			Map<DatapathId, List<OFStatsReply>> replies = getSwitchStatistics(switchService.getAllSwitchDpids(), OFStatsType.PORT); //Hashmap de los switches con cada estadistica del tipo port
			for (Entry<DatapathId, List<OFStatsReply>> e : replies.entrySet()) {
				for (OFStatsReply r : e.getValue()) { //Obtiene cada una de las respuestas para un determinado switch
					OFPortStatsReply psr = (OFPortStatsReply) r; //Cambia de tipo especificamente para el puerto
					for (OFPortStatsEntry pse : psr.getEntries()) { //De una respuesta obtiene las entries
						NodePortTuple npt = new NodePortTuple(e.getKey(), pse.getPortNo()); //no se que es tuple
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							//A partir de aqui se modifica el codigo para las estadisticas
							long traficoRx =  (rxBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec;
							long traficoTx =  (txBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec;
